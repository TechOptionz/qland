...
import {
  houseAndLandLocationsNode,
  houseAndLandProcessNode,
  pageGraph,
  serviceId,
  serviceNode,
} from "@/lib/schema";
...
export const metadata: Metadata = {
  ...
  openGraph: {
    ...
    images: [{ url: photos.facadeGarden, alt: "A completed QLand home with established landscaping" }],
  },
  twitter: {
    card: "summary_large_image",
    title: "House and Land · QLand Property",
    description: SOCIAL_DESCRIPTION,
    images: [photos.facadeGarden],
  },
};

/**
 * Three nodes carry this page: the service, the estates it covers (the local
 * signal that wins "house and land Park Ridge" style queries), and the
 * five-step process as a `HowTo` so an answer engine can quote the sequence.
 */
const service = serviceNode({
  path: "/house-and-land",
  name: "House and Land packages",
  description: houseAndLand.intro[0],
  serviceType: "House and land package",
});

const graph = pageGraph({
  path: "/house-and-land",
  name: "House and Land packages in South East Queensland",
  description: DESCRIPTION,
  image: photos.facadeGarden,
  trail: [{ name: "House and Land", path: "/house-and-land" }],
  mainEntityId: serviceId("/house-and-land"),
  extra: [service, houseAndLandLocationsNode, houseAndLandProcessNode],
});
          ...
          <ol className="mt-11 m-0 flex list-none flex-col gap-4 p-0">
            {houseAndLand.steps.map((step, i) => (
              <Reveal key={step.num} as="li" variant="up" delay={i * 60}>
                {/* Anchored so the `HowTo` step URLs in this page's structured
                    data resolve to the step they describe. */}
                <div
                  id={`step-${step.num}`}
                  className="card-lift grid grid-cols-1 gap-4 rounded-[18px] border border-line bg-white p-[clamp(22px,3vw,32px)] hover:border-amber hover:shadow-[0_14px_34px_rgba(240,166,60,0.12)] mdx:grid-cols-[auto_minmax(0,22ch)_minmax(0,1fr)] mdx:items-start mdx:gap-8"
                >
